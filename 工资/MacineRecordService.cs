...
public class MacineRecordService
{
    H3.IEngine engine;
    ...
    public MacineRecordService()
    {

    }
    public MacineRecordService(H3.IEngine engine, H3.SmartForm.SmartFormRequest request)
    {
        this.engine = engine;
        this.request = request;
        this.me = request.BizObject;
    }
    public MacineRecordService(H3.IEngine engine, BizObject bizObject)
    {
        this.engine = engine;
        this.me = bizObject;
    }
    ...
    public void UpdateMacineRecordTable(string processName, string objectID, string processPlanObjectid, H3.DataModel.BizObject automaticCal)
    {
        if (processName == "取样子流程")
        {
            UpdateMacineRecordSamplingSubFlow(objectID, processPlanObjectid, automaticCal);
        }
        if (processName == "粗车")
        {
            UpdateMacineRecordRoughing(objectID, processPlanObjectid, automaticCal);
        }
        if (processName == "精车")
        {
            UpdateMacineRecordFinishing(objectID, processPlanObjectid, automaticCal);
        }
        if (processName == "钻孔")
        {
            UpdateMacineRecordDrill(objectID, processPlanObjectid, automaticCal);
        }
    }
    public void AddTechHoursToRecord(string processName, string objectID, string processPlanObjectid)
    {
        if (processName == "粗车")
        {
            AddTechHoursToRecordRoughing(objectID, processPlanObjectid);
        }
        if (processName == "精车")
        {
            AddTechHoursToRecordFinishing(objectID, processPlanObjectid);
        }
        if (processName == "钻孔")
        {
            AddTechHoursToRecordDrill(objectID, processPlanObjectid);
        }
    }
    /*
    *--Author:zzx
    ...
    */
    public void UpdateMacineRecordSamplingSubFlow(string objectID, string processPlanObjectid, H3.DataModel.BizObject automaticCal)
    {
        //取样子流程
        H3.DataModel.BizObject samplingSubFlow = Tools.BizOperation.Load(engine, SamplingSubFlow_TableCode, objectID);
        //取样机加工子表
        H3.DataModel.BizObject[] samplingMachiningSubtable = samplingSubFlow[SamplingSubFlow_Sampling] as H3.DataModel.BizObject[];
        //产品参数表
        H3.DataModel.BizObject productParameterObj = Tools.Query.GetProductParameterFrom(engine, processPlanObjectid);
        //完成总量
        ...
        {
            //当前任务记录
            H3.DataModel.BizObject currentTask = samplingMachiningSubtable[taskNum];
            //加工者
            ...
            recordObj[MachiningTaskRecord_TaskType] = "正常取样";                        //任务类型
            ...
            if (productParameterObj != null)
            {
                ...
                recordObj[MachiningTaskRecord_MachiningProcessingCategory] = productParameterObj[ProductParameter.ProductMachiningCategory] + string.Empty;                 //车加工类别


                recordObj[MachiningTaskRecord_DrillingProcessingCategory] = productParameterObj[ProductParameter.ProductDrillingCategory] + string.Empty;                   //钻加工类别
                recordObj[MachiningTaskRecord_OrderSpecifications] = productParameterObj[ProductParameter.OrderSpecificationNumber] + string.Empty;                         //订单规格号
                recordObj[MachiningTaskRecord_FinishedProductUnitWeight] = productParameterObj[ProductParameter.FinishedProductUnitWeight] + string.Empty;                  //成品单重
                recordObj[MachiningTaskRecord_OutsideDiameter] = productParameterObj[ProductParameter.OuterDiameter] + string.Empty;                                        //工件外径
                recordObj[MachiningTaskRecord_InsideDiameter] = productParameterObj[ProductParameter.InnerDiameter] + string.Empty;                                         //工件内径
                recordObj[MachiningTaskRecord_TotalHeight] = productParameterObj[ProductParameter.TotalHeight] + string.Empty;                                              //工件总高
                recordObj[MachiningTaskRecord_Thickness] = productParameterObj[ProductParameter.SliceThickness] + string.Empty;                                             //工件片厚
                recordObj[MachiningTaskRecord_HoleAmount] = productParameterObj[ProductParameter.NumberOfHoles] + string.Empty;                                             //工件孔数
                recordObj[MachiningTaskRecord_Aperture] = productParameterObj[ProductParameter.HoleDiameter] + string.Empty;                                                //工件孔径
            }
            //加工开始时间
            ...
            recordObj[MachiningTaskRecord_PlanTable] = processPlanObjectid;

            recordObj.Create();
            currentTask["F0000161"] = recordObj.ObjectId;                                                                                                                   //任务加工记录
    ...
    */
    public void UpdateMacineRecordRoughing(string objectID, string processPlanObjectid, H3.DataModel.BizObject automaticCal)
    {
        //粗车
        ...
        //产品参数表
        H3.DataModel.BizObject productParameterObj = Tools.Query.GetProductParameterFrom(engine, processPlanObjectid);// Tools.BizOperation.Load(engine, ProductParameter_TableCode, productParameterId);
        //完成总量
        ...
        if (roughingMachiningObj != null)                                    //粗车下机
        {
            ...
            H3.DataModel.BizObject recordObj = Tools.BizOperation.New(engine, MachiningTaskRecord_TableCode);
            //设置为生效状态
            recordObj.Status = H3.DataModel.BizObjectStatus.Effective;
            //工序
            recordObj[MachiningTaskRecord_SectionName] = "粗车";
            if (roughingObj["F0000134"] + string.Empty == "已本取")           //完成取样  已取样
            {
                //任务类型
            ...
            //产品规格
            recordObj[MachiningTaskRecord_ProductSpecification] = productParameterObj[ProductParameter.ProductSpecification] + string.Empty;
    ...
    * @param objectID    工序表objectid
    */
    public void UpdateMacineRecordFinishing(string objectID, string processPlanObjectid, H3.DataModel.BizObject automaticCal)
    {
        //精车
        H3.DataModel.BizObject finishingObj = Tools.BizOperation.Load(engine, Finishing_TableCode, objectID);
        //精车机加表
        H3.DataModel.BizObject[] finishingMachiningObj = finishingObj[Finishing_Machining] as H3.DataModel.BizObject[];
        //产品参数表
        H3.DataModel.BizObject productParameterObj = Tools.Query.GetProductParameterFrom(engine, processPlanObjectid);// Tools.BizOperation.Load(engine, ProductParameter_TableCode, productParameterId);
                                                                                                                      //完成总量
        ...
        if (finishingMachiningObj != null)     //精车下机
        {
            //当前任务记录
            ...
            //加工开始时间
            DateTime startTime = recordObj[MachiningTaskRecord.StartTime] + string.Empty != string.Empty ?
                Convert.ToDateTime(recordObj[MachiningTaskRecord.StartTime] + string.Empty) : DateTime.Now;
            TimeSpan delayTime = DateTime.Now.Subtract(startTime);                                                  //与现在时间的差值
            recordObj[MachiningTaskRecord.ActualElapsedTime] = delayTime.TotalHours;                                //实际耗时
    ...
    * @param objectID    工序表objectid
    */
    public void UpdateMacineRecordDrill(string objectID, string processPlanObjectid, H3.DataModel.BizObject automaticCal)
    {
        //钻孔
        H3.DataModel.BizObject drillObj = Tools.BizOperation.Load(engine, Drill_TableCode, objectID);
        //钻孔车机加表
        H3.DataModel.BizObject[] drillMachiningObj = drillObj[Drill_Machining] as H3.DataModel.BizObject[];
        //产品参数表
        H3.DataModel.BizObject productParameterObj = Tools.Query.GetProductParameterFrom(engine, processPlanObjectid);// Tools.BizOperation.Load(engine, ProductParameter_TableCode, productParameterId);
                                                                                                                      //完成总量
        ...
        {
            //当前任务记录
            H3.DataModel.BizObject currentTask = drillMachiningObj[taskNum];
            //加工者
            H3.Organization.User processor = engine.Organization.GetUnit(currentTask["F0000143"] + string.Empty) as H3.Organization.User;
            //新建机加工记录数据对象
            H3.DataModel.BizObject recordObj = Tools.BizOperation.New(engine, MachiningTaskRecord_TableCode);
            ...
            recordObj[MachiningTaskRecord_MultipleDepartments] = drillObj["F0000185"];                                                                                    //部门多选
            recordObj[MachiningTaskRecord_PlanTable] = processPlanObjectid;
            recordObj.Create();
            currentTask["F0000144"] = recordObj.ObjectId;                                                                                                                  //当前任务加工记录
            automaticCal["F0000013"] = recordObj.ObjectId;                                                                                                                 //工资自动计算赋值 当前加工记录
    ...
    //粗车工序  工序记录表中添加工时和下屑量,注意：粗车的轧制方式影响工时，轧制方式不同，粗车工时不同。
    /*
    * @param engine 引擎
    ...
    **/
    public void AddTechHoursToRecordRoughing(string processObjectID, string processPlanObjectID, string plannedRollingMode = "单轧")
    {
        //粗车
        ...
        H3.DataModel.BizObject plan = Tools.BizOperation.Load(engine, ABCDProcessPlan_TableCode, processPlanObjectID);
        string plannedRollingModeFieldName = "F0000152";//轧制方式字段名
        plannedRollingMode = plan[plannedRollingModeFieldName] + string.Empty;
        //完成总量
        decimal totalAmountCompleted = roughingObj["F0000090"] + string.Empty != string.Empty ?
            decimal.Parse(roughingObj["F0000090"] + string.Empty) : 0;
        int taskNum = 0;
        //当前子表行数
        ...
        H3.DataModel.BizObject currentTask = roughingMachiningObj[taskNum];
        //加工记录
        H3.DataModel.BizObject recordObj = Tools.BizOperation.Load(engine, MachiningTaskRecord.TableCode, currentTask["F0000161"] + string.Empty);
        ...
        //设备工时系数
        double equipmentCoeff = th.EquipmentTechHoursCoeff(engine, "粗车", deviceType, productParameterObj, plannedRollingMode);
    ...
    * @param quantity  派工/加工量
    **/
    public void AddTechHoursToRecordFinishing(string processObjectID, string processPlanObjectID, string plannedRollingMode = "单轧")
    {
        //精车
        ...
        //产品参数表
        H3.DataModel.BizObject productParameterObj = Tools.Query.GetProductParameterFrom(engine, processPlanObjectID);
        // Tools.BizOperation.Load(engine, ProductParameter_TableCode, productParameterId);
        //完成总量
        decimal totalAmountCompleted = finishingObj["F0000086"] + string.Empty
            != string.Empty ? decimal.Parse(finishingObj["F0000086"] + string.Empty) : 0;
        int taskNum = 0;
        ...
        TechHours th = new TechHours();
        double equipmentCoeff = th.EquipmentTechHoursCoeff(engine, "精车", deviceType, productParameterObj, plannedRollingMode);                              //设备工时系数

        Dictionary<string, double> hoursAndChip = th.QueryTechHoursAndFilings(engine, "精车", productParameterObj, plannedRollingMode, equipmentCoeff, processQuantity);
        recordObj[MachiningTaskRecord_DeviceCoefficient] = equipmentCoeff;                                                                                    //设备工时系数
        recordObj[MachiningTaskRecord_NoDeviceTechHours] = hoursAndChip["NoDeviceTechHours"];                                                                 //无设备工时   
        recordObj[MachiningTaskRecord_TechHours] = hoursAndChip["techHours"];                                                                                 //精车工时 
        recordObj[MachiningTaskRecord_StageTechHours] = hoursAndChip["StageTechHours"];                                                                       //阶段加工任务工时
        recordObj[MachiningTaskRecord_FillingWeight] = hoursAndChip["FillingWeight"];                                                                         //任务下屑量
        recordObj.Update();
    }
    ...
    * @param quantity  派工/加工量
    **/
    public void AddTechHoursToRecordDrill(string processObjectID, string processPlanObjectID, string plannedRollingMode = "单轧")
    {
        //钻孔
        ...
        H3.DataModel.BizObject recordObj = Tools.BizOperation.Load(engine, MachiningTaskRecord_TableCode, currentTask["F0000144"] + string.Empty);        //加工记录 
        string deviceType = currentTask["F0000139"] + string.Empty;                                                                                       //设备类型 
        double processQuantity = Convert.ToDouble(currentTask["F0000140"]);                                                                               //加工量

        TechHours th = new TechHours();
        double equipmentCoeff = th.EquipmentTechHoursCoeff(engine, "钻孔", deviceType, productParameterObj, plannedRollingMode);                          //设备工时系数
        Dictionary<string, double> hoursAndChip = th.QueryTechHoursAndFilings(engine, "钻孔", productParameterObj, plannedRollingMode, equipmentCoeff, processQuantity);
        recordObj[MachiningTaskRecord_DeviceCoefficient] = equipmentCoeff;                                                                                //设备工时系数
        recordObj[MachiningTaskRecord_NoDeviceTechHours] = hoursAndChip["NoDeviceTechHours"];                                                             //无设备工时
        recordObj[MachiningTaskRecord_TechHours] = hoursAndChip["techHours"];                                                                             //钻孔工时
        recordObj[MachiningTaskRecord_StageTechHours] = hoursAndChip["StageTechHours"];                                                                   //阶段加工任务工时
        recordObj[MachiningTaskRecord_FillingWeight] = hoursAndChip["FillingWeight"];                                                                     //任务下屑量
        recordObj.Update();
    }
    ...
    string MachiningTaskRecord_PlanTable = "PlanTable";
